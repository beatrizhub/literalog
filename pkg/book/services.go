...
func (s *Service) CreateBook(book Book) (Book, error) {
	...
	query := "INSERT INTO books (title, authors, genre, description) VALUES ($1, $2, $3, $4) RETURNING id"

	_, err := s.Db.Exec(query, book.Title, pq.Array(book.Authors), pq.Array(book.Genre), book.Description)
	if err != nil {
		return Book{}, err
	}
...
func (s *Service) UpdateBook(id int, book Book) error {
	...
	query := "UPDATE books SET title = $1, authors = $2, genre = $3, description = $4 WHERE id = $5"

	_, err := s.Db.Exec(query, book.Title, pq.Array(book.Authors), pq.Array(book.Genre), book.Description, id)
	if err != nil {
		return err
	}
...
func (s *Service) GetBookByID(id int) (Book, error) {
	...
	err := s.Db.QueryRow(query, id).Scan(&book.ID, &book.Title, pq.Array(&book.Authors), pq.Array(&book.Genre), &book.Description)
	if err != nil {
		return Book{}, err
	}
...
func (s *Service) GetBooksRecommendations(userId int) ([]Book, error) {
	...
	query := "SELECT * FROM books WHERE genre && $1 OR authors && $2"
